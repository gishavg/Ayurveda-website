...
<?php
				...
				include 'connect.php';
				  $s=mysql_query("select     hospitalname,hospitaladdress,hospitallocation,phno1,phno2,hospitalemailid,hospitalwebsite,foodtype,environmentaldetails,responsibleperson,responsiblepersonphno,responsiblepersonemailid from hospitalregistration where hospitalid=".$_GET["id"]);
		...
		if(!$conn)
			...
			echo "not connected";
				...
				if($name=="Accepted")
				{
					...
					mysql_query($query);
					Header("Location:adminhospitalview.php");
				}
				else if($name=="Rejected")
				{
					$query2="delete from login where username=".$_GET["id"];
					mysql_query($query2);
